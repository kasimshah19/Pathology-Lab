...
export default function EditStaffModal({ open, onClose, staff, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setApiError('');
    try {
      await api.put(`/auth/users/${staff._id}`, form);
      onSuccess?.('Staff details updated successfully');
      onClose();
    } catch (err) {
      setApiError(err.response?.data?.message || 'Failed to update staff details');
    } finally {
      setSubmitting(false);
    }
  };
